				...
				if(!found)
				{
					if(pgNm == "Regulator")
					{
						if(obj.status == 0)
						{
							...
							obj.make = 'Zip Code';
							obj.model = 'Symptoms';
							obj.colour = 'Medicines';
							obj.reg = 'Disease Name';
							objects.push(obj);
						}
					}
					else
					...
					{
						error = true
						$("#vhclsTbl").append("Unable to load Disease.");
					}
				}
			}
		}
	...
	*/
	...
	xhr.onreadystatechange = function (){
		if(xhr.readyState === 4)
		{
			...
			{
				var data = d[i];
				if(data.VIN == 0) data.VIN = '&lt;<i>PIN</i>&gt;';
				if(data.make.toLowerCase() == 'undefined' || data.make.trim() == '') data.make = '&lt;<i>zip code</i>&gt;';
				if(data.model.toLowerCase() == 'undefined' || data.model.trim() == '') data.model = '&lt;<i>symptoms</i>&gt;';
				if(data.reg.toLowerCase() == 'undefined' || data.reg.trim() == '') data.reg = '&lt;<i>disease name</i>&gt;';
				if(data.colour.toLowerCase() == 'undefined' || data.colour.trim() == '') data.colour = '&lt;<i>medicines</i>&gt;';
