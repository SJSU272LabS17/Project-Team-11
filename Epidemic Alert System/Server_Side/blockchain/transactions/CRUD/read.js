...
function get_block(req, res, number) //Retrieves block, and retrieves transactions within the block
{
    ...
    request(options, function (error, response, body){
        if (!error && response.statusCode === 200) {
            ...
            alert("req.session.user  ",req.session.user)

            if(number + 1 < height) {
                get_block(req, res, number + 1);
            }
            else if (req.session.user === 'BA') {
                tracing.create('EXIT', 'GET blockchain/transactions', 'result');
                res.send(result);
            } else {
...
function evaluate_transactions(req, res)
{
        ...
        for(let i = 0; i < result.transactions.length; i++)
    {
            ...
            let diseaseID = transaction.payload.match(/[A-Z]{2}[0-9]{7}/g);
            if(JSON.stringify(transaction).indexOf(user_id) === -1 && validdiseases.indexOf(diseaseID) === -1)
        {
                result.transactions.splice(i, 1);
                i--;
            }
            else if (validdiseases.indexOf(diseaseID) === -1)
        {
                validdiseases += '['+diseaseID+']';
            }
        }
    }
